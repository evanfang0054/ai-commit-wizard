...
try {
   ...
   */
  execSync(GIT_ALIAS_COMMAND, {
    stdio: 'inherit',
    encoding: 'utf-8'
  });

  console.log(chalk.green('✨ Git别名配置成功! 现在你可以使用 git cw 来运行ai-commit-wizard'));
} catch (error) {
  // 错误处理与用户提示
  console.warn(
    chalk.yellow('⚠️ Git别名配置失败。你可以手动运行以下命令进行配置:\n') +
    chalk.cyan(`$ ${GIT_ALIAS_COMMAND}`)
  );
